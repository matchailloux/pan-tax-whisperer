...
-- Country normalization function (only create if doesn't exist)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'norm_country') THEN
    CREATE FUNCTION norm_country(raw text)
...
-- Sales events table (only create if doesn't exist)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'ventes_events_v1') THEN
    CREATE TABLE ventes_events_v1 (
